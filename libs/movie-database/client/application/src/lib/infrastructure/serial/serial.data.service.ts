...
export class SerialDataService {
  ...
  addSerial(value: SerialAddWriteModel): Observable<Serial> {
    return this.serialHttpService
      ...
      .pipe(
        ...
        tap(({ serial, actors }) => {
          this.serialStateManagerService.addSerial(serial, Object.values(actors));
        }),
        map(({ serial }) => serial)
      );
  }
  ...
  editSerial(id: SerialId, payload: SerialEditWriteModel): Observable<Serial> {
    ...
    return this.serialHttpService
      ...
      .pipe(
        tap((serial) =>
          this.serialStateManagerService.editSerial(serial, Object.values(actors))
        )
      );
  }
  ...
  private rememberResponse(
    response$: Observable<SerialWithActorsReadModel | SerialWithActorsReadModel[]>
  ): Observable<Serial[]> {
